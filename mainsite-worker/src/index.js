  ...
  try {
    ...
    for (const checkout of checkouts) {
      ...
      const effectiveStatus = resolveSumupStatusFromSources({
        ...
        payloadStatus: tx?.status || checkout.status || 'UNKNOWN',
      });
...
app.post('/api/sumup-payment/:id/refund', async (c) => {
  ...
  try {
    ...
    ).bind(id, newStatus, id, txnId).run();
...
app.put('/api/sumup-payment/:id/cancel', async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare(
      "UPDATE mainsite_financial_logs SET payment_id = ?, status = 'CANCELLED' WHERE method = 'sumup_card' AND (payment_id = ? OR payment_id = ?)"
    ).bind(id, id, transactionId).run();
...
// --- PIX PAYLOAD GENERATOR (Backend-only — key never exposed to frontend) ---

app.post('/api/pix/generate', async (c) => {
  try {
    const { amount: amountStr } = await c.req.json();
    ...
    structuredLog('error', 'PIX generate error', { error: err.message });
    return c.json({ error: err.message }, 500);
  }
});
