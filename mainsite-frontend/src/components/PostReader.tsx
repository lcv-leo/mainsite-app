...
const PostReader = ({ post, activePalette, settings, API_URL, onShare, onContact, onComment, onDonation, isSendingEmail, isNotHomePage, zoomLevel }: PostReaderProps) => {
  ...
  const handleSummarize = async () => {
    ...
    try {
      const [res] = await Promise.all([
        fetch(`${API_URL}/ai/workers/summarize`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: post.content }) }),
  ...
  const handleTranslate = async (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    try {
      const [res] = await Promise.all([
        fetch(`${API_URL}/ai/workers/translate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: post.content, lang }) }),
        new Promise<void>(resolve => setTimeout(resolve, 800))
      ]);
      const data = await res.json();
