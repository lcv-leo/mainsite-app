...
const DisclaimerItemView = ({ disclaimer, activePalette, isDarkBase, totalItems, positionIndex, dontShowAgain, setDontShowAgain, onAgree, onSkipDonation }: DisclaimerItemViewProps) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: '480px', maxHeight: 'min(90vh, 720px)', background: surfaceBg, backdropFilter: 'blur(var(--glass-blur-deep))', WebkitBackdropFilter: 'blur(var(--glass-blur-deep))', border: `1px solid rgba(${isDarkBase ? '255,255,255' : '0,0,0'}, 0.15)`, borderRadius: '28px', padding: 'clamp(20px, 4vw, 36px)', boxShadow: '0 32px 64px -12px rgba(0,0,0,0.4)', color: activePalette.fontColor, textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '18px', animation: 'fadeIn 0.4s ease-out', textShadow: isDarkBase ? '0 1px 3px rgba(0,0,0,0.35)' : 'none', overflow: 'hidden', minHeight: 0 }}>

      {totalItems > 1 && (
        <div style={{ position: 'absolute', top: '16px', right: '20px', fontSize: '12px', fontWeight: 800, opacity: 0.5, letterSpacing: '1px' }}>
          {positionIndex + 1} / {totalItems}
        </div>
      )}

      <div style={{ display: 'flex', justifyContent: 'center', color: isDonationMode ? '#ec4899' : activePalette.titleColor, opacity: 0.9, flex: '0 0 auto' }}>
        {isDonationMode ? <Heart size={44} /> : <AlertTriangle size={44} />}
      </div>
      <h3 id="disclaimer-title" style={{ margin: 0, fontSize: '22px', color: activePalette.titleColor, textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 800, flex: '0 0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', flex: '0 0 auto' }}>
        ...
        <button
          ...
          style={{ padding: '14px 28px', background: primaryBg, color: primaryFg, border: 'none', borderRadius: '100px', cursor: canClose ? 'pointer' : 'not-allowed', fontWeight: 800, letterSpacing: '2px', transition: 'all 0.2s', textTransform: 'uppercase', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: canClose ? `0 8px 24px ${isDonationMode ? 'rgba(236, 72, 153, 0.4)' : activePalette.titleColor + '40'}` : 'none', opacity: canClose ? 1 : 0.45, filter: canClose ? 'none' : 'saturate(0.6)' }}
          onMouseOver={(e) => { if (canClose) e.currentTarget.style.transform = 'translateY(-2px)'; }}
        ...
        {isDonationMode && (
          <button
            ...
            disabled={!canClose}
            aria-disabled={!canClose}
            style={{ background: 'transparent', border: 'none', color: activePalette.fontColor, fontSize: '13px', opacity: canClose ? 0.6 : 0.3, cursor: canClose ? 'pointer' : 'not-allowed', textDecoration: 'underline', fontWeight: 700, marginTop: '4px' }}
          >
...
const DisclaimerModal = ({ show, onClose, activePalette, config, onDonationTrigger }: DisclaimerModalProps) => {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="disclaimer-title" aria-describedby="disclaimer-body" style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 11000, padding: '16px' }}>

      <div key={`bg-${currentIndex}`} style={{ position: 'absolute', inset: 0, backgroundColor: isDarkBase ? 'rgba(15,15,20,0.85)' : 'rgba(240,240,244,0.56)', backdropFilter: 'blur(var(--glass-blur-subtle))', WebkitBackdropFilter: 'blur(var(--glass-blur-subtle))', animation: 'fadeIn 0.3s ease-out' }}></div>
