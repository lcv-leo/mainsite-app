...
- `useTextZoomVoice.ts`: `VoiceCommand.command` ajustado para `RegExp`.
- `TEXT_ZOOM_DEMO.ts`: retirado do repo por inflar diagnósticos do workspace sem participar do build.
### Adicionado
- Text zoom finalizado no `PostReader` com floating control, atalhos, presets e hooks auxiliares para cloud sync, analytics e voz.
### Controle de versão
